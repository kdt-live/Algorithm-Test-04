...
for i in range(1, T+1):
    ...
    matrix = [list(map(int, sys.stdin.readline().split())) for j in range(N)]
    graph = [[] for k in range(N ** 2)]

    for l in range(N):
        for m in range(N-1):
            if matrix[l][m] > matrix[l][m+1]:
                graph[l * N + m + 1].append(l * N + m)
            elif matrix[l][m] < matrix[l][m+1]:
                graph[l * N + m].append(l * N + m + 1)
            if matrix[m][l] > matrix[m+1][l]:
                graph[m * N + l + 1].append(m * N + l)
            elif matrix[m][l] < matrix[m+1][l]:
                graph[m * N + l].append(m * N + l + 1)
